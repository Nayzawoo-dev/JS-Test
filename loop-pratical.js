...
let total2 = 0;
const totalFun = function (numArr) {
  for (x of numArr) {
    total2 += x;
  }
  console.log(total2);
...
const evenOrOdd = function (numArr) {
  for (x of numArr) {
    if (x % 2 == 0) {
      Evenarr[arrIndex1++] = x;
    } else {
...
console.log(evenOrOdd(points));
// 4. array 5ခန်း ခွဲထုတ်ပြပါ
const splitArr = [];
let index = 0;
const splitArray = function (numArr) {
  for (x in numArr) {
    if (x == 5) {
      break;
    }
    splitArr[index++] = numArr[x];
  }
  return splitArr;
};
console.log(splitArray(points));

// 5. ကြိုက်တဲ့ index ကစ ကြိုက်သလောက် အခန်းအရည်အတွက် ခွဲထုတ် နိုင်တဲ့ function
const splitIndex = function (numArr, z, y) {
  const index = [];
  let indexNum = 0;
  for (x in numArr) {
    //    for(x=z;x<z+y;x++){
...
console.log(splitIndex(points, 2, 4));
console.log(splitIndex([10, 20, 30, 40, 50], 1, 3));

// 6. CopyArray, Modified Array, Filter Array

const copyArr = function (numArr) {
  const copy = [];
  let copyindex = 0;
  for (x of numArr) {
    copy[copyindex++] = x;
